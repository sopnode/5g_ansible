...
class CSIVisualizer:
    ...
    def plot_prb_magnitude(self, slot_idx, prb_idx, symbol_idx=None, port_idx=None):
        """Plot magnitude vs subcarrier for a specific PRB (all symbols/ports by default)"""
        records = [r for r in self.parser.records
                   if r.slot_idx == slot_idx and r.prb_idx == prb_idx]
        
        # Filter by symbol/port if specified
        ...
        plt.title(f'CSI Magnitude - Slot {slot_idx}, PRB {prb_idx} ({len(records)} samples)')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        return plt.gcf()

    def plot_prb_phase(self, slot_idx, prb_idx, symbol_idx=None, port_idx=None):
        """Plot phase vs subcarrier for a specific PRB (all symbols/ports by default)"""
        records = [r for r in self.parser.records
                   if r.slot_idx == slot_idx and r.prb_idx == prb_idx]
        ...
        plt.title(f'CSI Phase - Slot {slot_idx}, PRB {prb_idx} ({len(records)} samples)')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        return plt.gcf()
    ...
    def plot_constellation(self, slot_idx, prb_idx, symbol_idx=None, port_idx=None):
        ...
        plt.xlabel('I')
        plt.ylabel('Q')
        plt.title(f'CSI Constellation - Slot {slot_idx}, PRB {prb_idx} ({len(records)} samples)')
        plt.grid(True, alpha=0.3)
        plt.axis('equal')
        plt.tight_layout()
        return plt.gcf()
    ...
    def plot_prb_heatmap(self, slot_idx):
        ...
        # Create matrix: PRB x Subcarrier (average over symbols/ports)
        ...
        for i, prb in enumerate(prbs):
            for record in records_by_prb[prb]:
                ...
                counts[i, record.subcarrier_in_prb] += 1
        ...
        plt.title(f'CSI Magnitude Heatmap - Slot {slot_idx} (averaged over symbols/ports)')
        plt.tight_layout()
        return plt.gcf()
    ...
    def plot_timeline(self, prb_idx=0, symbol_idx=None, port_idx=None):
        ...
        plt.xlabel('Time (ms)')
        plt.ylabel('Magnitude')
        plt.title(f'CSI Magnitude Timeline - PRB {prb_idx} ({len(records)} samples)')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        return plt.gcf()
...
def main():
    ...
    parser.add_argument('--symbol', type=int, default=None, help='Symbol index (optional)')
    parser.add_argument('--port', type=int, default=None, help='Port index (optional)')
